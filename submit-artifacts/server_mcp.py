...
# -*- coding: utf-8 -*-
"""
MCP Stock Analysis Server + Agentic Reasoning
---------------------------------------------
 Adds reasoning step between draft → critique
 If reasoning concludes analysis is conclusive or highly negative, it halts early
 Full RAG + LangGraph + MCP + Rich UI + Visualization
"""
...
def build_graph(ticker,max_headlines=5):
    ...
    def reasoning_node(s):
        ...
        decision = generator_pipeline(reasoning_prompt, max_new_tokens=30)[0]["generated_text"]
        s["reasoning_decision"] = decision.strip()
        console.print(f"[warn] Reasoning decision for {ticker}: {decision.strip()}")
        return s
    ...
    #  Conditional agentic reasoning branch
    g.add_conditional_edges(
        "reasoning",
        lambda s: "critique" if "YES" in s.get("reasoning_decision","").upper() else END,
...
def _analyze_stock_impl(ticker,max_headlines=5):
    ...
    if "reasoning_decision" in state:
        console.print(f"[warn]Reasoning:[/] {state['reasoning_decision']}")
